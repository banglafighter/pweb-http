import requests
from pweb_http.phttp.phttp_const import PHTTPConst
...
class HTTPResponse:
    ...
    def process_response(self, response) -> "HTTPResponse":
        ...
        if 200 <= self.httpCode < 300:
            self.status = PHTTPConst.SUCCESS
        else:
            self.status = PHTTPConst.ERROR
        self.set_data(response)
        return self

    def set_data(self, response):
        response.encoding = PHTTPConst.UTF8_ENCODING
        self.contentType = response.headers["Content-Type"]
        if self.contentType and self.contentType.lower().startswith(PHTTPConst.APPLICATION_JSON):
